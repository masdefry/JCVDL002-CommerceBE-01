...
const profPictUpload = require("./../Helpers/ProfPictUpload")();
...
const register = async (req, res) => {
  ...
  try {
    ...
    if (checkEmail.length > 0)
      ...
      }; // Apabila data yang di dapat dari query1 lebih dari nol, maka kita anggap email sudah terdaftar
    ...
    let dataToSend = {
      ...
      profile_picture_url: data.profile_picture_url
        ...
        : "\\userProfilePicture\\USER_default_photo.jpg",
      status: "deactive",
      created_at: new Date().toISOString().slice(0, 19).replace("T", " "),
    };

    const insertData = await query(query2, dataToSend).catch((error) => {
      throw error;
    });

    const getDataUser = await query(query3, insertData.insertId).catch(
      (error) => {
        throw error;
      }
    );
    ...
    if (token) {
      let getSendEmailVerification = await sendEmailVerification(
        data.email
      ).catch((error) => {
        throw error;
      });
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (data.username) {
      ...
      getDataUser = await query(query1, data.username).catch((error) => {
        throw error;
      });
    } else if (data.email) {
      getDataUser = await query(query2, data.email).catch((error) => {
        throw error;
      });
    }
...
const getUserProfile = async (req, res) => {
  ...
  let query1 = "SELECT * FROM users WHERE idusers = ?";
  try {
    await query("Start Transaction");
    const getDataUser = await query(query1, data.id).catch((error) => {
      throw error;
    });
    ...
    res.status(200).send({
      ...
      data: {
        ...
        gender: getDataUser[0].gender,
        status: getDataUser[0].status,
        profile_picture: getDataUser[0].profile_picture_url,
      },
    });
  } catch (error) {
    if (error.status) {
...
const updateUser = async (req, res) => {
  try {
    const profPictUploadAwait = util
      .promisify(profPictUpload)
      .bind(profPictUpload);
    await profPictUploadAwait(req, res);
    ...
    if (req.files.length === 0 || req.files === undefined) {
      let query1 = "SELECT * FROM users WHERE idusers = ?";
      let query2 =
        "UPDATE users SET fullname = ?, dob = ?, gender = ? WHERE idusers = ?";
      ...
      try {
        ...
        let updatePasswordUser = await query(query2, [
          dataToSend.fullname,
          dataToSend.dob,
          dataToSend.gender,
        ...
        res.status(200).send({
          ...
          detail: "Update Data User Dilakukan!",
          data: {
            id: getDataUser[0].id,
            username: getDataUser[0].username,
            email: getDataUser[0].email,
      ...
      let file = req.files;
      let filePath = file[0].path.slice(6);

      let query1 = "SELECT * FROM users WHERE idusers = ?";
      let query2 =
        "UPDATE users SET fullname = ?, dob = ?, gender = ?, profile_picture_url = ? WHERE idusers = ?";
      ...
      try {
        ...
        let dataToSend = {
          ...
          dob: dataBody.dob ? dataBody.dob : "",
          gender: dataBody.gender ? dataBody.gender : "",
          profile_picture_url: filePath ? filePath : "",
        };
        ...
        let updatePasswordUser = await query(query2, [
          ...
          dataToSend.dob,
          dataToSend.gender,
          dataToSend.profile_picture_url,
          data.id,
        ]).catch((error) => {
          throw error;
        });
        ...
        res.status(200).send({
          error: false,
          message: "Update user Success",
          detail: "Update Data + Photo User Dilakukan!",
          data: {
            id: getDataUser[0].id,
            username: getDataUser[0].username,
            email: getDataUser[0].email,
...
const changePassword = async (req, res) => {
  ...
  try {
    ...
    const getDataUser = await query(queryUser, paramGetUser).catch((error) => {
      throw error;
    });

    const updatePasswordUser = await query(query2, [
      dataBody.newPassword,
      getDataUser[0].id,
    ]).catch((error) => {
      throw error;
    });
...
const sendEmailLink = async (req, res) => {
  ...
  try {
    await query("Start Transaction");
    const getDataUser = await query(queryGetUser, email).catch((error) => {
      throw error;
    });
    ...
    if (getDataUser[0].status == "deactive") {
      ...
      transporter
        .sendMail({
          ...
          html:
            '<p>You requested for email verification, kindly use this <a href="http://localhost:5000/user/verifyEmail?token=' +
            token +
        ...
        .then((response) => {
          if (response) {
            ...
            query(queryMagicLink, tokenObj).catch((error) => {
              throw error;
            });
          }
        })
    ...
    if (getDataUser[0].status == "deactive") {
      ...
      transporter
        .sendMail({
          ...
          subject: "Email verification - warehouse.com",
          html:
            '<p>You requested for email verification, kindly use this <a href="http://localhost:5000/verifyEmail?token=' +
            token +
            '">link</a> to verify your email address</p>',
        })
        .then((response) => {
...
const verifyEmail = async (req, res) => {
  ...
  try {
    ...
    const getMagicLink = await query(queryGetMagicLink, token).catch(
      (error) => {
        throw error;
      }
    );

    const activateUser = await query(queryActivate, getMagicLink[0].id).catch(
      (error) => {
        throw error;
      }
    );
...
const createAddress = async (req, res) => {
  ...
  try {
    ...
    const insertData = await query(query1, dataToSend).catch((error) => {
      throw error;
    });

    const getAddress = await query(query2, data.id).catch((error) => {
      throw error;
    });
...
const getCurrentAddress = async (req, res) => {
  ...
  try {
    await query("Start Transaction");
    const getDataAddressById = await query(query1, data.id).catch((error) => {
      throw error;
    });
    await query("Commit");
...
const updateDeafultAddress = async (req, res) => {
  ...
  let query1 =
    "UPDATE addresses_by_user SET is_default = ?  WHERE idaddress = ?";
  let query2 =
    "UPDATE addresses_by_user SET is_default = ? WHERE idaddress = ? AND is_default = 1";
  let query3 =
    "SELECT * FROM addresses_by_user WHERE idusers = ? AND is_default = 1";
  let query4 = "SELECT * FROM addresses_by_user WHERE idusers = ?";

  try {
    await query("Start Transaction");
    const getDefaultCurrentAddress = await query(query3, data.id).catch(
      (error) => {
        throw error;
      }
    );
    if (getDefaultCurrentAddress.length > 0) {
      const updateCurrentAddress = await query(query2, [
        0,
        getDefaultCurrentAddress[0].idaddress,
      ]).catch((error) => {
        throw error;
      });
    }
    const updateNextAddress = await query(query1, [
      1,
      dataBody.idaddress,
    ]).catch((error) => {
      throw error;
    });
    const getDataAddressById = await query(query4, data.id).catch((error) => {
      throw error;
    });
    await query("Commit");
